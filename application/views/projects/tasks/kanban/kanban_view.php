<div id="kanban-wrapper" class="row">
        ...
        foreach ($tasks as $task) {
            ...
            $item = $exising_items .  modal_anchor(get_uri("projects/task_view"), 
                        ...
                        $task_labels,
                    array("class"=>"kanban-item", "data-id"=>$task->id, "data-project_id"=>$task->project_id, "data-sort"=>$task->sort, "data-post-id" => $task->id, "title" => lang('task_info') . " #$task->id",));
...
<script type="text/javascript">
    ...
    saveStatusAndSort = function ($item, status) {
        ...
        } else if (prevSort && prevSort) {
            //item moved inside two items
            newSort = (prevSort + prevSort) / 2;
